#!/usr/bin/env python3
"""
Film Roll Inventory Format Converter
...
python3 scripts/inventory_format_converter/inventory_format_converter.py --input in/in.md --output out/out.md

Run internal tests:
python3 scripts/inventory_format_converter/inventory_format_converter.py --self-test
"""
...
class FilmEntry:
    ...
    - section_path: Tracks the hierarchical section path (e.g., ["new", "b/w"])
    """
    ...
    roll_num: str = ""
    quantity: str = ""  # e.g., "1x"
    section_path: list[str] = dataclasses.field(default_factory=list)  # e.g., ["new", "b/w"]
    ...
    def to_markdown(self) -> str:
        ...
        - Filmstock: VALUE
            ...
            - Developed Location: VALUE (empty for new rolls, defaults to "Citizens PDX" when developed)
            - RollNum: VALUE
        """
        ...
        def empty_for_new_roll(value: str) -> str:
            """Return empty string for fields that wouldn't exist on new unshot rolls."""
            return value if value.strip() else ""

        def none_for_new_roll(value: str) -> str:
            """Return 'None' for fields that should show 'None' even on new rolls."""
            return value if value.strip() else "None"

        # Fields that wouldn't exist for new unshot rolls should be empty instead of "None"
        # Developed Location should also be empty for new rolls (not default to "Citizens PDX")
        developed_location_val = empty_for_new_roll(self.developed_location)
        ...
        lines = [
            ...
            f"    - Loaded Date: {empty_for_new_roll(self.loaded_date)}",
            f"    - Camera: {empty_for_new_roll(self.camera)}",
            f"    - Lens: {empty_for_new_roll(self.lens)}",
            f"    - Filter: {empty_for_new_roll(self.filter)}",
            f"    - Notes: {or_default(self.notes)}",
            f"    - Subject: {empty_for_new_roll(self.subject)}",
            f"    - Shot Location: {empty_for_new_roll(self.shot_location)}",
            f"    - Ready for Development Date: {empty_for_new_roll(self.ready_date)}",
            f"    - Developed Date: {empty_for_new_roll(self.developed_date)}",
            f"    - Developed Location: {developed_location_val}",
            f"    - RollNum: {empty_for_new_roll(self.roll_num)}",
        ]
        return "\n".join(lines)
...
class CatalogConverter:
    ...
    def _finalize_entry(self, e: FilmEntry) -> None:
        ...
        if e.camera:
            ...
            if canon is None:
                # Unknown camera; preserve raw to notes but leave camera empty for new rolls
                e.notes = (
                    f"{e.notes}; Camera (raw): {e.camera}".strip("; ").strip()
                    if e.notes
                    else f"Camera (raw): {e.camera}"
                )
                e.camera = ""  # Leave empty instead of "Unknown" for new rolls
            else:
                e.camera = canon
        else:
            ...
            # else: leave camera empty for new rolls (don't set to "Unknown")
    ...
    def parse_entries(self, content: str) -> list[FilmEntry]:
        ...
        Section headings are tracked and preserved in the section_path field.
        """
        ...
        for ln, raw in enumerate(lines, start=1):
            ...
            if line.startswith("## "):
                current_section_path = [line[3:].strip()]
            elif line.startswith("### "):
                current_section_path = [line[4:].strip()]
            elif line.startswith("#### "):
                if len(current_section_path) >= 1:
                    current_section_path = current_section_path[:1] + [line[5:].strip()]
                else:
                    current_section_path = [line[5:].strip()]
                    ...
                    if re.match(r"^-\s+Filmstock\b", trimmed, flags=re.IGNORECASE) or re.match(
                        ...
                        if current_is_new_format:
                            ...
                            if qm:
                                ...
                                current.filmstock = val
                            current.section_path = current_section_path.copy()
                        else:
                            m = re.match(
        ...
        try:
            ...
            if "expiration" in lcl or lcl.startswith("expires"):
                ...
                if not raw_val:
                    ...
                    pass
                elif "unknown" in raw_val.lower():
                    # Preserve unknown/likely dates in expiration field, don't move to notes
                    entry.expiration = raw_val
                else:
        ...
        try:
            ...
            if key == "iso":
                ...
                if not val:
                    # No expiration info provided
                    pass
                elif "unknown" in val.lower():
                    # Preserve unknown/likely dates in expiration field
                    entry.expiration = val
                else:
                ...
                if re.match(r"^(?:around|at|in)\b", val, flags=re.IGNORECASE):
                    ...
                    # Normalize to known cameras or leave empty for new rolls
                    ...
                    if cam_canon is None:
                        ...
                        entry.camera = ""  # Leave empty instead of "Unknown" for new rolls
                    else:
    ...
    async def convert_file(self, input_path: Path, output_path: Path) -> bool:
        ...
        try:
            ...
            # Group entries by section path and render output
            sections: dict[tuple[str, ...], list[FilmEntry]] = {}
            for e in entries:
                section_key = tuple(e.section_path) if e.section_path else ("uncategorized",)
                if section_key not in sections:
                    sections[section_key] = []
                sections[section_key].append(e)

            # Render output organized by sections
            blocks: list[str] = []
            for section_key in sorted(sections.keys()):
                section_entries = sections[section_key]

                # Add section headings
                if len(section_key) >= 1 and section_key[0] != "uncategorized":
                    blocks.append("## current inventory")
                if len(section_key) >= 2:
                    blocks.append(f"### {section_key[1]}")
                if len(section_key) >= 3:
                    blocks.append(f"#### {section_key[2]}")

                # Add entries in this section
                for i, e in enumerate(section_entries):
                    if i > 0:
                        blocks.append("")
                    blocks.append(e.to_markdown())

                # Add spacing between sections
                if section_key != list(sections.keys())[-1]:
                    blocks.append("")
...
"""
            ...
            # Expiration undetermined => preserved in field, not moved to notes
            self.assertEqual(e.expiration, "unknown, likely expired")
            md = e.to_markdown()
            self.assertIn("- Expiration: unknown, likely expired", md)
            self.assertNotIn("Expiration info:", md)  # Should not be in notes

            # Developed Location is empty for new rolls (not defaulted to Citizens PDX)
            self.assertIn("- Developed Location: ", md)
...
"""
            ...
            self.assertEqual(len(entries), 1)
            e = entries[0]
            # Expiration unknown should be preserved in field, not moved to notes
            self.assertEqual(e.expiration, "expiration unknown, likely 2026")
            md = e.to_markdown()
            self.assertIn("- Expiration: expiration unknown, likely 2026", md)
            # Notes should not contain expiration info (it should be in the expiration field)
            self.assertNotIn("Expiration info:", md)
...
"""
            ...
            self.assertEqual(e.developed_location, "Citizens PDX")
            md = e.to_markdown()
            # Empty developed date -> empty (not "None") for new rolls
            self.assertIn("- Developed Date: ", md)
